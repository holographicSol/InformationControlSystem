...
rawUserDirectory = (mainDir+'/Raw-Indexes/raw-d6_directory-index.py')
csvUserDirectory = (mainDir+'/CSV-Indexes/csv-d6-directory-index.py')
...
def write_index():
    global dir_target_d6
    global dir_target_root_d6
    global live_path
    global indexed_path
    global write_request
    print('index d6 directory: writing...')
    refreshIndex = open(rawUserDirectory, "w").close()
    for dirname, dirnames, filenames in os.walk(dir_target_d6):
        for subdirname in dirnames:
            fullpath = os.path.join(dir_target_root_d6, dirname, subdirname)
    ...
    ifile.close()
    ofile.close()
    print('index d6 directory: wrote.')
    write_request = False
    ...
    with open(config, 'r') as fo:
        for line in fo:
            ...
            if line.startswith('DRIVE6:'):
                dir_target_d6 = line.replace('DRIVE6:', '')
                dir_target_d6 = dir_target_d6.strip()
                dir_target_root_d6 = str(dir_target_d6.split('\\')[0]+'\\')
                # print(dir_target_root_d6, dir_target_d6)
...
while 1 == 1:
    ...
    if not os.path.exists(csvUserDirectory):
        open(csvUserDirectory, 'w').close()
    get_config()
    if os.path.exists(dir_target_d6):
        get_live_paths()
        get_indexed_paths()
